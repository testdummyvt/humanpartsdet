...
def sort_labels_by_image_id(labels_list: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    for i, labels_dict in tqdm(enumerate(labels_list), desc="Sorting labels by image_id"):
...
def normalize_bbox(bbox: Tuple[float, float, float, float], img_w: int, img_h: int) -> Tuple[float, float, float, float]:
    """
...
def convert_pbbox_to_yolo(bbox: List[float], img_w: int, img_h: int) -> Tuple[float, float, float, float]:
    """
    ...
    """
    if bbox[0] + bbox[2] > img_w: bbox[2] = img_w - 1 - bbox[0]
    if bbox[1] + bbox[3] > img_h: bbox[3] = img_h - 1 - bbox[1]
    if bbox[0] < 0:
        bbox[2] += bbox[0]
        bbox[0] = 0
    if bbox[1] < 0:
...
def convert_hpbbox_to_yolo(bbox: List[float], img_w: int, img_h: int) -> Tuple[float, float, float, float]:
    """
...
def process_coco_human_parts(anno_data_file: str, images_dir: str, dst_images_dir: str, dst_labels_dir: str) -> None:
    """
    ...
    for image_info in tqdm(images_info, desc="Processing images"):
        ...
        dst_label_path = os.path.join(dst_labels_dir, img_file_name.replace(img_ext, 'txt'))
...
if __name__ == '__main__':
    ...
    # process train data
    process_coco_human_parts(train_anno_file, train_image_dir, dst_train_image_dir, dst_train_label_dir)

    # process val data
    process_coco_human_parts(val_anno_file, val_image_dir, dst_val_image_dir, dst_val_label_dir)